...
def _build_reflection_schemas(runner, phase: str = "") -> list[dict]:
    from tools.reflection import (
        ...
        build_update_correlation_schema,
    ...
    if phase == "l1_reflection":
        ...
        tools.append({"type": "function", "function": build_update_correlation_schema(all_agent_names)})
...
def _build_reflection_handlers(runner) -> dict:
    ...
    _L2_TOOLS = {"update_teammate_profile", "update_correlation", "skip_l2_reflection"}
    _L3_CHAIR_TOOLS = {
    ...
    def _check_phase(tool_name: str, caller_name: str) -> str | None:
        ...
        if phase == "l2_reflection":
            if tool_name in _L2_TOOLS:
                return None
            return f"Error: '{tool_name}' is not available during L2 reflection. Use L2 tools: update_teammate_profile, update_correlation, skip_l2_reflection."
    ...
    async def _update_correlation(
        partner_name: str = "", suggestion: str = "",
        _caller_name: str = "", **kwargs,
    ) -> str:
        err = _check_phase("update_correlation", _caller_name)
        if err:
            return err
        err = _check_already_completed("update_correlation", _caller_name)
        if err:
            return err
        agent_dir = _get_agent_dir(_caller_name)
        if not agent_dir:
            return f"Error: no agent directory found for {_caller_name}."
        result = execute_update_correlation(
            partner_name=partner_name, suggestion=suggestion,
            _agent_name=_caller_name, _agent_dir=agent_dir,
        )
        return result + (
            "\n\nCall skip_l2_reflection when you are done with L2 reflection."
    ...
    return {
        ...
        "update_correlation": _update_correlation,
...
REFLECTION_TOOL_NAMES = frozenset({
    ...
    "update_teammate_profile", "update_correlation", "skip_l2_reflection",
    "propose_reflection", "view_reflection", "view_current_config",
    "apply_reflection", "skip_reflection",
