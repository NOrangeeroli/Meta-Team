#!/usr/bin/env python3
"""Validate the frozen Meta-Team three-layer evolution smoke run."""
...
EXPERIMENT_ID = "20260812_qwen_plus_three_layer_smoke_v1"
...
def main() -> int:
    ...
    if len(run_dirs) != 1:
        print(json.dumps({"status": "FAIL", "error": f"expected exactly one run directory, found {len(run_dirs)}"}, indent=2))
        return 1
    run_dir = run_dirs[0]
    ...
    gates["G1_real_model_tool_path"] = {
        "pass": llm_models == {"qwen-plus"} and len(tool_calls) > 0,
