...
crossrepo_holdout() {
    ...
    python3 -c "print(','.join(str(i) for i in range(20, 200)))"
}

depmigrate_holdout() {
    # DepMigrate: 178 total, idx 20-177 = 158 holdout
    python3 -c "print(','.join(str(i) for i in range(20, 178)))"
}
...
phase_evolve() {
    ...
    local run_id="${TS}_bswe_${split}_evolve"
    ...
    run_cmd "BeyondSWE $split evolve (20 cases, serial)" \
        python3 benchmarks/adapter_beyondswe.py \
            --split "$split" \
            --cases "$EVOLVE_CASES" \
            --team pool_BeyondSWE \
            --evolve \
    ...
    log "  1. bash scripts/freeze_evolved_team.sh ${run_id}_evolve latest pool_BeyondSWE_evolved_${split}"
