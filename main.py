...
def copy_team_to_session(session_dir: Path, source_team_dir: Path) -> Path:
    """Copy team config to session directory for reproducibility."""
...
def _save_trace(session_id: str, session_dir: str) -> None:
    ...
    try:
        from core.audit import generate_audit
        generate_audit(session_id, quiet=True, session_dir=session_dir)
    except Exception as e:
...
async def run_task(task: str, team_name: str = DEFAULT_TEAM, template: str | None = None,
    ...
    try:
        runner = Runner(
            ...
            max_seconds=float(pool.settings.get("max_seconds", 600)),
